...
const finalAngles = [-2, 1.5, -1, 2.5, -3];
...
export default function HeroSection() {
    ...
    const ctx = gsap.context(() => {
      ...
      if (cards.length > 0) {
        ...
        cardTl.to(
          ...
          {
            duration: 0.8,
            rotationZ: (i: number) => finalAngles[i] || 0,
            ease: 'power2.out',
            stagger: { each: 0.04, from: 'center' },
          },
          '-=0.6'
  ...
  return (
    ...
    >
      ...
      {/* 3D Memory Card Cluster */}
      <div
        ref={cardsRef}
        className="absolute z-10 perspective-1000 hidden md:block"
        style={{
          bottom: '8%',
          left: '50%',
      ...
      {/* Mobile card row */}
      <div
        className="absolute z-10 flex md:hidden items-end justify-center gap-3"
        style={{ bottom: '12%', left: 0, right: 0, padding: '0 16px' }}
      >
        {heroPhotos.slice(0, 3).map((photo, i) => (
          <div
            key={photo.id}
            style={{
              transform: `rotate(${finalAngles[i]}deg) translateY(${i % 2 === 0 ? 0 : -10}px)`,
            }}
          >
            <PolaroidCard
              src={photo.src}
